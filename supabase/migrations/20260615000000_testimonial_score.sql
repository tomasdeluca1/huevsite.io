-- Builder score: +50 por dejar un testimonio (status != 'rejected').
-- Incentiva la prueba social que alimenta la home, y se muestra en el modal de
-- score con link a /testimonio.
--
-- Re-aplica recompute_builder_score y get_builder_score_breakdown agregando la
-- categoría 'testimonial', y recalcula a todos. CREATE OR REPLACE — reversible
-- re-aplicando 20260609000002.
--
-- Nota: la tabla public.testimonials ya está en prod (flujo de testimonios live
-- desde 2026-06-09). Si por algún entorno no existiera, esta migración fallaría;
-- aplicarla recién después de testimonials.
...
BEGIN
  ...
  -- Testimonio: +50 una vez por dejar uno que no esté rechazado.
  SELECT EXISTS (
...
BEGIN
  ...
  -- Testimonio: +50 una vez por dejar uno no rechazado.
  SELECT EXISTS (
    SELECT 1 FROM public.testimonials
    WHERE user_id = target_user_id AND status <> 'rejected'
  ) INTO v_has_testimonial;
...
-- Recalcular a todos con la nueva categoría de testimonio.
