...
import { Trophy, Loader2, CheckCircle2, AlertCircle, Trash2, Archive, RotateCcw, X, LogIn, Share2, Mic, Send, Eye, Check } from "lucide-react";
...
export default function AdminPage() {
  ...
  const searchBuilders = async (q: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/builder-interview/search?q=${encodeURIComponent(q)}`);
      const json = await res.json();
  ...
  const handleInviteBuilder = async (username: string) => {
    ...
    try {
      ...
      const json = await res.json();
      if (res.ok) {
        setInterviewMsg({ type: "ok", msg: `Invitación enviada a @${username} ✉️` });
        fetchInterviews();
      } else {
        setInterviewMsg({ type: "err", msg: json.error || "Error al invitar." });
      }
    } catch {
      setInterviewMsg({ type: "err", msg: "Error de conexión." });
    } finally {
  ...
  const handleApproveInterview = async (id: string) => {
    setApprovingId(id);
    try {
      const res = await fetch(`/api/admin/builder-interview/${id}/approve`, { method: "POST" });
      const json = await res.json();
      if (res.ok) {
    ...
    <div className="min-h-screen bg-[var(--bg)] font-display py-12 px-4 max-w-3xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-10">
          ...
          {/* Builder de la Semana Section */}
          <div className="pt-10 border-t border-[var(--border)]">
            <div className="flex items-center justify-between mb-6">
              <div className="section-label !mb-0">// builder de la semana</div>
              <button
            ...
            {/* Invite builder — autocomplete dropdown */}
                  ...
                  {showDropdown && searchResults.length > 0 && (
                    <div className="absolute z-50 top-full left-0 right-0 mt-2 bg-[#111] border border-[var(--border)] rounded-2xl overflow-hidden shadow-2xl">
                      {searchResults.map((builder: any) => (
                        ...
                        >
                          {builder.image ? (
                            <img src={builder.image} alt="" className="w-8 h-8 rounded-full object-cover" />
                          ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-bold text-white truncate">{builder.name || builder.username}</div>
                            <div className="text-[11px] text-[var(--text-muted)] font-mono">@{builder.username}</div>
                          </div>
            ...
            {interviewMsg && (
              <div className={`text-sm font-mono mb-4 ${interviewMsg.type === "ok" ? "text-green-400" : "text-red-400"}`}>
                {interviewMsg.msg}
              </div>
            )}

            {/* Interviews list — 3 estados: Pendiente, En proceso, Finalizado */}
            {loadingInterviews ? (
              <div className="text-center py-8">
                <Loader2 size={20} className="animate-spin text-[var(--accent)] mx-auto" />
              </div>
            ) : interviews.length === 0 ? (
              <p className="text-[var(--text-muted)] text-sm text-center py-8">No hay entrevistas todavía.</p>
            ) : (
              <div className="space-y-3">
                {interviews.map((iv: any) => {
                  const isPendiente = iv.status === "invited";
                  const isEnProceso = iv.status === "submitted" || iv.status === "generating";
                  const isFinalizado = iv.status === "ready" || iv.status === "published";
                  const isExpired = iv.status === "expired";
                  const isExpanded = expandedId === iv.id;

                  return (
                    <div key={iv.id} className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl overflow-hidden">
                      {/* Row header */}
                      <div
                        className={`p-5 flex items-center justify-between ${isFinalizado ? "cursor-pointer hover:bg-white/[0.02] transition-colors" : ""}`}
                        onClick={() => isFinalizado && setExpandedId(isExpanded ? null : iv.id)}
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                            isFinalizado ? (iv.status === "published" ? "bg-green-400" : "bg-[var(--accent)]") :
                            isEnProceso ? "bg-amber-400 animate-pulse" :
                            isExpired ? "bg-red-400" :
                            "bg-white/20"
                          }`} />
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-bold text-white truncate">{iv.builder_name}</span>
                              <span className="text-xs text-[var(--text-muted)] font-mono shrink-0">@{iv.builder_username}</span>
                            </div>
                            <div className="text-[11px] text-[var(--text-muted)] mt-0.5">
                              {iv.invited_at && `Invitado ${new Date(iv.invited_at).toLocaleDateString("es-AR", { day: "numeric", month: "short" })}`}
                              {iv.submitted_at && ` · Respondió ${new Date(iv.submitted_at).toLocaleDateString("es-AR", { day: "numeric", month: "short" })}`}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <span className={`text-[10px] font-mono px-3 py-1 rounded-full uppercase tracking-wider ${
                            ...
                            iv.status === "ready" ? "bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20" :
                            isEnProceso ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" :
                      ...
                      {/* Expanded content for Finalizado */}
                        ...
                        <div className="border-t border-[var(--border)] px-5 pb-5">
                          ...
                          <div className="flex items-center gap-2 py-4">
                            ...
                            {iv.status === "ready" && (
                              ...
                              >
                                {approvingId === iv.id ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />}
                                Publicar blog
                              </button>
                            )}
                            {iv.status === "published" && (
