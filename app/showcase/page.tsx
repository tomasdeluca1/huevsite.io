    ...
    <div className="min-h-screen bg-[var(--bg)] font-display">
      <main className="max-w-4xl mx-auto px-4 pt-12 pb-24">
        ...
        <div className="mb-16 text-center">
          <div className="section-label mb-3 justify-center">{t("sectionLabel")}</div>
          <h1 className="text-5xl font-extrabold tracking-tighter mb-4">
            {t("title")}
          </h1>
          <p className="section-sub mx-auto max-w-md mb-6">
            {t("subtitle")}<br />
            {data?.winners?.[0]?.week && <span className="font-mono text-xs text-[var(--accent)]">{t("edition", { week: data.winners[0].week })}</span>}
          </p>

          <div className="inline-flex flex-col items-center p-4 rounded-2xl bg-[var(--surface)] border border-[var(--border)]">
            <span className="text-xs text-[var(--text-dim)] font-mono uppercase mb-2">{t("nextWinnerIn")}</span>
            <Countdown />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-center text-[var(--text-dim)] font-mono">{t("errorLoading")}</p>
        ) : (
          <>
            {/* Winners */}
            {data.winners && data.winners.length > 0 ? (
              <div className="space-y-8 mb-16">
                {data.winners.map((winner, idx) => (
                  <motion.div
                    key={`${winner.user.id}-${idx}`}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="relative p-8 rounded-[2.5rem] border-2 overflow-hidden"
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-8">
                      ...
                      <div className="flex-1 text-center md:text-left">
                        <div
                          ...
                          style={{ backgroundColor: winner.user.accent_color }}
                        >
                          {t("winnerLabel", { week: winner.week })}
                        </div>
              ...
              <div className="mb-16 p-12 rounded-[2.5rem] border border-dashed border-[var(--border-bright)] text-center">
                ...
                <p className="text-[var(--text-dim)] font-mono text-sm">
                  {t("noWinnerYet")}<br />
                  <span className="text-[var(--accent)]">{t("nominateFavorite")}</span>
                </p>
              </div>
            )}
            ...
            {data.finalists.length > 0 && (
              <div>
                <div className="section-label mb-6">{t("nominationsOfWeek", { week: data.week })}</div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[var(--text-muted)] font-mono">
                          {t("nominationsCount", { count: finalist.count })}
                        </p>
                      </div>
