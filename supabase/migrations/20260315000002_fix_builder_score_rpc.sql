-- Fix: Change owner_id to user_id in builder score functions
...
BEGIN
  ...
  -- J. Diversidad de contenido bonus
  ...
  -- G. Pro Power-Ups (FIXED: owner_id -> user_id)
  ...
  END IF;
  SELECT EXISTS (SELECT 1 FROM public.sub_sites WHERE user_id = target_user_id AND custom_domain IS NOT NULL AND custom_domain != '') INTO v_has_custom_domain;
  IF v_has_custom_domain THEN v_score := v_score + 150; END IF;

  -- H. Visibilidad (Visitas únicas 30d)
...
DECLARE
  ...
  v_has_github BOOLEAN; v_has_project BOOLEAN := false; v_has_building BOOLEAN := false; v_has_writing BOOLEAN := false;
  ...
  v_sub_sites_count INTEGER := 0; v_has_custom_domain BOOLEAN := false; v_unique_visitors INTEGER := 0;
  v_block RECORD;
BEGIN
  SELECT name, tagline, image, github_handle, pro_since, updated_at INTO v_profile FROM public.profiles WHERE id = target_user_id;
  ...
  SELECT EXISTS (SELECT 1 FROM public.blocks WHERE user_id = target_user_id AND visible = true AND type = 'github') INTO v_has_github;
  IF v_has_github OR (v_profile.github_handle IS NOT NULL AND v_profile.github_handle != '') THEN v_score_content := v_score_content + 150; END IF;
  ...
  -- Pro Gaming (FIXED: owner_id -> user_id)
  SELECT count(*) INTO v_sub_sites_count FROM public.sub_sites WHERE user_id = target_user_id;
  IF v_sub_sites_count >= 1 THEN v_score_pro_gaming := v_score_pro_gaming + 80 + (LEAST(v_sub_sites_count - 1, 4) * 40); END IF;
  SELECT EXISTS (SELECT 1 FROM public.sub_sites WHERE user_id = target_user_id AND custom_domain IS NOT NULL AND custom_domain != '') INTO v_has_custom_domain;
  IF v_has_custom_domain THEN v_score_pro_gaming := v_score_pro_gaming + 150; END IF;
  ...
  v_result := jsonb_build_object(
    ...
    'breakdown', jsonb_build_object(
      ...
      'content', jsonb_build_object('score', v_score_content, 'details', jsonb_build_object('project_count', v_project_count, 'building_count', v_building_count, 'writing_count', v_writing_count, 'has_github', v_has_github OR (v_profile.github_handle IS NOT NULL AND v_profile.github_handle != ''), 'content_diversity_bonus', v_has_project AND v_has_building AND v_has_writing)),
      ...
      'pro_gaming', jsonb_build_object('score', v_score_pro_gaming, 'details', jsonb_build_object('sub_sites_count', v_sub_sites_count, 'has_custom_domain', v_has_custom_domain)),
