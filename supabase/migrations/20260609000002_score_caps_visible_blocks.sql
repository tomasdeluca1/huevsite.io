-- Builder score must reflect what's PUBLICLY visible, not every block the user
-- flagged visible=true. A free user capped at 5 (+ share bonus) blocks should
-- only earn content points for those 5 — same as their public profile renders.
-- Previously both score functions looped over ALL visible=true blocks, so a
-- downgraded Pro (e.g. @azanello: 12 blocks built as Pro, now free → only 5
-- shown) kept a Pro-sized score.
--
-- Fix: cap the block loop + the github check to the first N visible blocks by
-- "order", where N = effective public limit:
--   pro access  → all blocks (effectively uncapped)
--   free        → 5 + extra_blocks_from_share
-- "pro access" mirrors lib/pro-access.ts hasProAccess (paid/lifetime/referral/
-- active free-trial). The block order mirrors profile-service (order ASC).
-- Re-applies to BOTH recompute_builder_score and get_builder_score_breakdown,
-- then bulk-recomputes everyone. CREATE OR REPLACE — reversible by re-applying
-- the previous migration (20260327000001).
...
BEGIN
  ...
  -- Effective public block cap (mirrors hasProAccess + free cap).
...
-- Recompute everyone with the visible-block cap.
